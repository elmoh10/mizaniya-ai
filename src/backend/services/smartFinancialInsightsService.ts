...
export async function buildSmartFinancialInsights(userId:string) {
  ...
  return {today,monthKey,income:money(income),expenses:money(expenses),netCashFlow:money(income-expenses),walletBalance:money(walletBalance),dailyBurn:money(dailyBurn),safeToSpend:money(safe),safeDaily:money(safeDaily),projectedRemainingSpend:money(projectedSpend),remainingCommitments:money(commitments),projectedMonthEndBalance:money(projectedBalance),risk,topCategory:top?{category:top[0],amount:money(top[1])}:null,goalsCount:goals.length,goalsProgressPercent:Math.round(goalsProgress*100),savingsRatePercent:Math.round(savingsRate*100),timeline};
}
