...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const parseResult = profileOnboardingSchema.safeParse(
        req.body
      );
      ...
      const defaultWallet =
        await ensureDefaultWalletForUser(userId);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const profile =
        await profileRepository.getProfile(userId);
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const context =
        await getTrustedFinancialContext(userId);
      ...
      const plan = await buildSmartBudgetPlan(
        context,
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const incomeStabilityScore =
        context.historicalIncomeStability
          .calculatedScore;
...
//
// This route is intentionally located AFTER authMiddleware.
// The user must be authenticated with Firebase before a
// Telegram account can be attached to their Mizaniya account.
// ============================================================
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          error:
            'كود الربط يجب أن يكون مكوّنًا من 6 أرقام.',
        });
      }
      ...
      await db.runTransaction(
        async (transaction) => {
          ...
          if (
            ...
            throw new Error(
              'TELEGRAM_LINK_CODE_ALREADY_USED'
            );
          }

          if (
            typeof codeData.expiresAt !==
              'number' ||
            Date.now() >
              codeData.expiresAt
          ) {
          ...
          if (
            !codeData.telegramUserId ||
            !codeData.chatId
          ) {
            throw new Error(
              'TELEGRAM_LINK_CODE_INVALID'
            );
          }
          ...
          transaction.set(
            telegramLinkRef,
            {
              ...
              telegramUsername:
                codeData.telegramUsername ||
                null,

              telegramFirstName:
                codeData.telegramFirstName ||
                null,

              telegramLastName:
                codeData.telegramLastName ||
                null,
      ...
      return res.status(200).json({
        success: true,
        message:
          'تم ربط حساب Telegram بحساب Mizaniya AI بنجاح.',
      });
    } catch (error: any) {
      console.error(
        'Telegram account linking error:',
        error
      );
        ...
        case 'TELEGRAM_LINK_CODE_NOT_FOUND':
          return res
            ...
            .json({
              success: false,
              error:
                'كود الربط غير صحيح. اطلب كود جديد من Telegram.',
            });
        ...
        case 'TELEGRAM_LINK_CODE_ALREADY_USED':
          return res
            ...
            .json({
              success: false,
              error:
                'تم استخدام كود الربط ده بالفعل.',
            });
        ...
        case 'TELEGRAM_LINK_CODE_EXPIRED':
          return res
            ...
            .json({
              success: false,
              error:
                'انتهت صلاحية الكود. اكتب /link في Telegram للحصول على كود جديد.',
            });
