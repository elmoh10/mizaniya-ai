...
import telegramRoutes from './src/backend/routes/telegramRoutes';
...
// Telegram Bot Routes
// IMPORTANT:
// Telegram does NOT authenticate using Firebase ID Tokens.
// Therefore Telegram routes are mounted separately from /api/v1.
// ============================================================
...
// ============================================================
// Mizaniya API
// CORS is scoped to API traffic.
// Authentication remains handled inside apiRoutes.
// ============================================================
...
app.get('/ready', async (req, res) => {
  ...
  const responseBody = {
    ...
    services: {
      ...
      firebaseAdmin: adminInitialized
        ? 'initialized'
        : 'failed',
...
async function startServer() {
  ...
  const server = app.listen(
    ...
    () => {
      console.log(
        `🚀 Mizaniya AI Server running on http://0.0.0.0:${PORT}`
      );
    }
  );
  ...
  const gracefulShutdown = (signal: string) => {
    ...
    setTimeout(() => {
      console.error(
        'Forcing shutdown after 10s timeout.'
      );
